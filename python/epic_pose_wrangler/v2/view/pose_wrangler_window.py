...
class PoseWranglerWindow(MayaQWidgetDockableMixin, QtWidgets.QMainWindow):
    ...
    event_mirror_solver = QtCore.Signal(object, str, str)
    ...
    event_mirror_pose = QtCore.Signal(str, object, str, str)
    ...
    def _mirror_solver(self):
        ...
        mirror_axes = self._get_mirror_axes()
        for solver in self._get_selected_solvers():
            self.event_mirror_solver.emit(solver, *mirror_axes)
    ...
    def _get_mirror_axes(self):
        if self.win.mirror_rot_x_RADIO.isChecked():
            mirror_rotation_axis = "x"
        elif self.win.mirror_rot_y_RADIO.isChecked():
            mirror_rotation_axis = "y"
        else:
            mirror_rotation_axis = "z"

        if self.win.mirror_trans_x_RADIO.isChecked():
            mirror_translation_axis = "x"
        elif self.win.mirror_trans_y_RADIO.isChecked():
            mirror_translation_axis = "y"
        else:
            mirror_translation_axis = "z"

        return mirror_rotation_axis, mirror_translation_axis
    ...
    def _mirror_pose(self):
        ...
        mirror_axes = self._get_mirror_axes()
        for pose_name in poses:
            self.event_mirror_pose.emit(pose_name, solver, *mirror_axes)
